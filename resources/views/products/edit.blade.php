...
<div class="col-md-12">
<div class="card mb-4">  
    <div class="card-header text-center text-success"><i class="fa fa-pencil" aria-hidden="true"></i> Edit Product</div>
    <div class="card-body">
...
<form method="POST" action="{{ url('products/'.$product->id)  }}" role="form" enctype="multipart/form-data">
    {{ method_field('PATCH') }}
    @csrf
<div class="form-row">
        ...
        <div class="form-group col-md-4">
            ...
            <input id="slug" type="text" class="form-control{{ $errors->has('slug') ? ' is-invalid' : '' }}" name="slug" value="{{ $product->slug }}" required>
            @if ($errors->has('slug'))
                <span class="invalid-feedback" role="alert">
...
<div class="form-row">
    <div class="form-group col-md-4">
        ...
        @endif
        <span class="form-help">Has</span>
        <span class="text-danger shop-discount-calculate">0%</span>
        <span class="">discount for this products</span>
    </div>
    ...
    <div class="form-group{{ $errors->has('discount_id') ? ' has-error' : '' }} col-md-4 mt-2">
        <label for="discount_id">{{ __('Discount Price') }}</label>
            ...
            <option {{ ( $product->discount_id  == $discount->id ) ? 'selected' : null }} value="{{ $discount->id }}">{{ $discount->discount }}</option>
            @endforeach
            @endif
            </select>
    ...
    <div class="form-group{{ $errors->has('category_id') ? ' has-error' : '' }} col-md-4 mt-2">
            ...
            <select id="category_id" class="form-control" name="category_id" required>
                ...
                <option <?php echo ( $product_cat->id == $category->id) ? 'selected' : null; ?> value="{{ $category->id }}">{{ $category->name }}</option>
                @endforeach
                @endforeach
                @endif
            </select>
    ...
    <div class="form-row">
        ...
        <div class="form-group col-md-4 mt-2">
            ...
            <input id="quantity" type="text" class="form-control{{ $errors->has('quantity') ? ' is-invalid' : '' }}" name="quantity" value="{{ $product->quantity }}">
    ...
    <div class="form-group col-md-4">
        <label for="image">{{ __('Image Preview') }}</label> <br>
        <img src="{{ asset($product->getImagePath()) }}" alt="product" class="img-fluid" width="150" height="50">
    </div>
    </div>
...
if($('#discount_id option:selected').length){
    let shop = $('#price').val();
...
$("#discount_id").on('change', function () {
    let check_selected = $('#discount_id option:selected').length;
    let shop = $('#price').val();
    let discount = parseFloat($('#discount_id option:selected').text());
    if (discount > 0) {
        if (shop == '' || shop == 0 || shop === "NaN") {
